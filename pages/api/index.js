...
const getData = async (get_date, total_cases, total_deaths, current_hospitalized, current_icu, cases, deaths, death_rate_7, cumulative_fatality_rate, new_tests, vaccines_administered, people_fully_vaccinated) => {
    ...
    fs.writeFile('./api/data/total_cases.json', JSON.stringify(total_cases_to_write), (err) => {
        if (err) throw err;
    ...
    const lastFunc = async () => {
        return `data successfully updated at ${new Date().toLocaleString()}`;
    }
    return await lastFunc();
}
